...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Api Generator account.<br>
        ...
        &mdash; Api Generator Team
      `
      return sendMail({ toEmail: email, subject: 'Api Generator - Password Reset', content })
    })
